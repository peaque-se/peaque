...
function createHandler(asset: AssetData, enableLongCache: boolean = false): RequestHandler {
  return async (req) => {
    ...
    // Set cache headers for immutable assets (hashed folders)
    if (enableLongCache) {
      req.header("cache-control", "public, max-age=31536000, immutable")
    }
