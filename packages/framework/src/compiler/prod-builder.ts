...
async function extractHeadStacks(router: RouteNode<string>, basePath: string, pagesDir: string, assetPath: string): Promise<Map<string, { headStack: string[], html: string }>> {
...
export const buildForProduction = async (basePath: string, distFolder: string, minify: boolean,
  ...
  // Create build hash from frontend JS content
  const buildHash = createHash("sha1").update(frontendJs).digest("hex").substring(0, 8)
  const assetDir = path.join(outDir, `assets-${buildHash}`)
  ...
  if (analyze && result.metafile?.inputs) {
    ...
    const projectFiles: Array<{ file: string; size: number }> = []

    for (const [inputPath, inputInfo] of Object.entries(result.metafile.inputs)) {
    ...
    for (const [pkg, size] of sortedPackages.slice(0, 15)) {
      ...
      const bar = "█".repeat(Math.ceil(parseInt(percentage) / 2))
  ...
  const headStacks = await extractHeadStacks(frontend, basePath, basePath + "/src/pages", assetPath)
